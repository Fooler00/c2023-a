...
Myprint::Node_Tree *Myprint::createroot() {
    ...
    memcpy(head->Board_, chessboard, sizeof(chessboard));
...
Myprint::Node_Tree *Myprint::createleaf(Node_Tree *p)  {
    ...
    tmp->Alpha=p->Alpha;
    tmp->Beta=p->Beta;
    memcpy(tmp->Board_, p->Board_, sizeof(p->Board_));
    tmp->Board_[p->site[p->cnt].Y][p->site[p->cnt].X] = p->flag;
    if (chessjudge(p->site[p->cnt].Y, p->site[p->cnt].X, tmp->Board_)) {
        if (tmp->depth % 2 == 0) {
            p->cnt++;
